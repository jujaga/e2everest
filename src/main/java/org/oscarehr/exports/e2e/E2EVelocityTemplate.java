...
public class E2EVelocityTemplate extends VelocityTemplate {
	...
	private static final String buildTag = "buildTag";
	...
	private static ClinicDao clinicDao = PatientExport.getApplicationContext().getBean(ClinicDao.class);
	private Clinic clinic = clinicDao.find(Constants.Runtime.VALID_CLINIC);
	...
	public String export(PatientExport p) {
		...
		// Create Data Model
		velocityContext.put("patient", record);
		velocityContext.put("e2e", e2eResources);
		velocityContext.put("stringUtils", stringUtils);
		velocityContext.put("custodian", clinic);
		velocityContext.put("randDocGUID", randDocGUID);
		velocityContext.put("buildTag", buildTag);
		velocityContext.put("notSupported", notSupported);
		velocityContext.put("noInformation", noInformation);

		// Merge Template & Data Model
		String result = VelocityUtils.velocityEvaluate(velocityContext, template);
